        ...
        try:
            ...
            if st.button("Processar e Salvar Importação"):
                ...
                for index, row in df_import.iterrows():
                    ...
                    try:
                        # Usa o Pandas para converter (mais inteligente com formatos variados)
                        # dayfirst=True ajuda a identificar que 01/02 é 1 de Fev, não 2 de Jan
                        data_obj = pd.to_datetime(data_raw, dayfirst=True, errors='coerce')
                        if pd.isnull(data_obj): # Se não conseguiu converter (NaT)
