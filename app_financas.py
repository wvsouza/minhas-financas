...
with tab2:
    ...
    # --- Inicialização dos Campos do Formulário (Estado) ---
    # Isso permite que o QR Code preencha os campos automaticamente
    if 'form_tipo' not in st.session_state: st.session_state.form_tipo = "Despesa"
    if 'form_data' not in st.session_state: st.session_state.form_data = datetime.now()
    ...
    # --- Leitor de QR Code ---
    with st.expander(" Importar via Link (QR Code do Celular)"):
        st.info("Utilize a câmera nativa do seu celular para ler o QR Code, copie o link gerado e cole abaixo:")
        qr_url_input = st.text_input("Cole o Link da Nota Fiscal aqui")
        
        if st.button("Preencher com Link"):
            if qr_url_input:
                st.session_state.form_tipo = "Despesa"
                st.session_state.form_desc = f"Nota Fiscal: {qr_url_input}"
                st.success("Link processado! Complete o valor e salve.")
                st.rerun()
    ...
    with st.form("form_manual"):
        tipo_operacao = st.radio("Tipo", ["Despesa", "Receita"], horizontal=True, key="form_tipo")
        
        col_a, col_b = st.columns(2)
        # Input de data com formato brasileiro
        data_transacao = col_a.date_input("Data", format="DD/MM/YYYY", key="form_data")
        valor = col_b.number_input("Valor (R$)", min_value=0.0, format="%.2f", key="form_valor")
        ...
        descricao = st.text_input("Descrição", placeholder="Ex: Padaria", key="form_desc")
        ...
        if submitted and valor > 0:
            ...
            # Reseta os campos do formulário para o padrão
            st.session_state.form_valor = 0.0
            st.session_state.form_desc = ""
            st.session_state.form_data = datetime.now()
