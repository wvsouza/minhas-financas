...
with tab2:
    ...
    # --- Leitor de QR Code ---
    with st.expander("📸 Ler QR Code da Nota Fiscal (Automático)"):
        img_file = st.camera_input("Tire uma foto do QR Code da Nota")
        ...
        if img_file is not None:
            ...
            if data:
                ...
                st.session_state.form_desc = f"Nota Fiscal: {data}"
                st.rerun()
            else:
                st.warning("Não foi possível identificar um QR Code na imagem. Tente aproximar mais.")
